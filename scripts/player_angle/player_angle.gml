...
function player_angle_control(){
	...
	if !sideway{
		if !place_meeting(x,y+4*gravdir,_solid) and !global.pause{
		    if global.playmode == 0{
				p_angle += 6;//change those if room speed changed
			}else if global.playmode == 1{
				p_angle += 6*abs(input.move_right-input.move_left);//change those if room speed changed
			}
		}
		
		if place_meeting(x,y+4*gravdir,_solid){
		...
		if !place_meeting(x+4*gravdir,y,_solid) and !global.pause{
		    if global.playmode == 0{
				p_angle += 6;//change those if room speed changed
			}else if global.playmode == 1{
				p_angle += 6*abs(input.move_right-input.move_left);//change those if room speed changed
		...
		if place_meeting(x+4*gravdir,y,_solid){
		    if p_angle < 450 and p_angle > 270{
		        player_resetAngle(360);
		    }else if p_angle < 360 and p_angle > 180{
		        player_resetAngle(270);
